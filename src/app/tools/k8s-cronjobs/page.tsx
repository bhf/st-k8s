      ...
      <table className="min-w-full border border-zinc-200 dark:border-zinc-700">
        <thead>
          <tr className="bg-zinc-100 dark:bg-zinc-800">
            ...
            <th className="px-4 py-2 text-left">Suspend</th>
            <th className="px-4 py-2 text-left">Active</th>
            <th className="px-4 py-2 text-left">Last Schedule</th>
        ...
        <tbody>
          {data.map((row, i) => (
            <tr key={i} className="border-t border-zinc-200 dark:border-zinc-700">
              ...
              <td className="px-4 py-2">{row.suspend ? 'True' : 'False'}</td>
              <td className="px-4 py-2">{row.active}</td>
              <td className="px-4 py-2">{row.lastScheduleTime ? new Date(row.lastScheduleTime).toLocaleString() : '-'}</td>
              <td className="px-4 py-2">{row.created ? new Date(row.created).toLocaleDateString() : '-'}</td>
            </tr>
          ))}
          {data.length === 0 && (
            <tr>
                <td colSpan={6} className="px-4 py-8 text-center text-zinc-500">No cronjobs found in this namespace</td>
            </tr>
          )}
        </tbody>
      </table>
...
function CronJobsFetcher({ namespace }: { namespace: string }) {
  const { data, error } = useSWR(`/api/tools/k8s-cronjobs?namespace=${namespace}`, fetcher, {
      suspense: true,
      fallbackData: { data: [] }
  });
  if (error) return <div className="text-red-600">Error: {error.message}</div>;
  if (!data || !data.data) return <div>No data found.</div>;
  return <CronJobsTable data={data.data} />;
}
...
export default function K8sCronJobsPage() {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-zinc-50 font-sans dark:bg-black">
      <main className="flex w-full max-w-4xl flex-col gap-8 p-8 bg-white dark:bg-zinc-900 rounded-xl shadow-lg">
        <h1 className="text-3xl font-bold mb-2 text-black dark:text-white">K8s CronJobs</h1>
        <form
        ...
        <section className="flex flex-col gap-4">
          <h2 className="text-lg font-semibold text-zinc-700 dark:text-zinc-300">CronJobs List</h2>
          <Suspense fallback={<div className="text-zinc-500">Loading cronjobs...</div>}>
            <CronJobsFetcher key={`${namespace}-${fetchKey}`} namespace={namespace} />
          </Suspense>
        </section>
      </main>
    </div>
