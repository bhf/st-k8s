      ...
      <table className="min-w-full border border-zinc-200 dark:border-zinc-700">
        <thead>
          <tr className="bg-zinc-100 dark:bg-zinc-800">
            ...
            <th className="px-4 py-2 text-left">Data Count</th>
        ...
        <tbody>
          {data.map((row, i) => (
            <tr key={i} className="border-t border-zinc-200 dark:border-zinc-700">
              ...
              <td className="px-4 py-2">{row.namespace}</td>
              <td className="px-4 py-2">{row.dataCount}</td>
              <td className="px-4 py-2">{row.created ? new Date(row.created).toLocaleDateString() : '-'}</td>
            </tr>
          ))}
          {data.length === 0 && (
            <tr>
                <td colSpan={4} className="px-4 py-8 text-center text-zinc-500">No ConfigMaps found in this namespace</td>
            </tr>
          )}
        </tbody>
      </table>
...
function ConfigMapsFetcher({ namespace }: { namespace: string }) {
  const { data, error } = useSWR(`/api/tools/k8s-configmaps?namespace=${namespace}`, fetcher, {
      suspense: true,
      fallbackData: { data: [] }
  });
  if (error) return <div className="text-red-600">Error: {error.message}</div>;
  if (!data || !data.data) return <div>No data found.</div>;
  return <ConfigMapsTable data={data.data} />;
}
...
export default function K8sConfigMapsPage() {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-zinc-50 font-sans dark:bg-black">
      <main className="flex w-full max-w-4xl flex-col gap-8 p-8 bg-white dark:bg-zinc-900 rounded-xl shadow-lg">
        <h1 className="text-3xl font-bold mb-2 text-black dark:text-white">K8s ConfigMaps</h1>
        <form
        ...
        {fetchKey >= 0 && (
          <Suspense fallback={<div className="text-gray-500">Loading ConfigMaps...</div>}>
            <ConfigMapsFetcher key={namespace + fetchKey} namespace={namespace} />
          </Suspense>
        )}
      </main>
    </div>
