...
function ReplicaSetsTable({ data }: { data: ReplicaSet[] }) {
  return (
      ...
      <table className="min-w-full border border-zinc-200 dark:border-zinc-700">
        <thead>
          <tr className="bg-zinc-100 dark:bg-zinc-800">
            ...
            <th className="px-4 py-2 text-left">Desired</th>
        ...
        <tbody>
          {data.map((row, i) => (
            <tr key={i} className="border-t border-zinc-200 dark:border-zinc-700">
              ...
              <td className="px-4 py-2">{row.desired}</td>
           ...
           {data.length === 0 && (
            <tr>
                <td colSpan={5} className="px-4 py-8 text-center text-zinc-500">No replicasets found in this namespace</td>
            </tr>
          )}
        </tbody>
      </table>
...
function ReplicaSetsFetcher({ namespace }: { namespace: string }) {
  const { data, error } = useSWR(`/api/tools/k8s-replicasets?namespace=${namespace}`, fetcher, {
      suspense: true,
      fallbackData: { data: [] }
  });
  if (error) return <div className="text-red-600">Error: {error.message}</div>;
  if (!data || !data.data) return <div>No data found.</div>;
  return <ReplicaSetsTable data={data.data} />;
}
...
export default function K8sReplicaSetsPage() {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-zinc-50 font-sans dark:bg-black">
      <main className="flex w-full max-w-4xl flex-col gap-8 p-8 bg-white dark:bg-zinc-900 rounded-xl shadow-lg">
        <h1 className="text-3xl font-bold mb-2 text-black dark:text-white">K8s ReplicaSets</h1>
        <form
        ...
        {fetchKey >= 0 && (
          <Suspense fallback={<div className="text-gray-500">Loading replicasets...</div>}>
            <ReplicaSetsFetcher key={namespace + fetchKey} namespace={namespace} />
          </Suspense>
        )}
      </main>
    </div>
