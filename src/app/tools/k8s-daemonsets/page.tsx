      ...
      <table className="min-w-full border border-zinc-200 dark:border-zinc-700">
        ...
        <tbody>
          {data.map((row, i) => (
           ...
           {data.length === 0 && (
            <tr>
                <td colSpan={6} className="px-4 py-8 text-center text-zinc-500">No daemonsets found in this namespace</td>
            </tr>
          )}
        </tbody>
      </table>
...
function DaemonSetsFetcher({ namespace }: { namespace: string }) {
  const { data, error } = useSWR(`/api/tools/k8s-daemonsets?namespace=${namespace}`, fetcher, {
      suspense: true,
      fallbackData: { data: [] }
  });
  if (error) return <div className="text-red-600">Error: {error.message}</div>;
  if (!data || !data.data) return <div>No data found.</div>;
  return <DaemonSetsTable data={data.data} />;
}
...
export default function K8sDaemonSetsPage() {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-zinc-50 font-sans dark:bg-black">
      <main className="flex w-full max-w-4xl flex-col gap-8 p-8 bg-white dark:bg-zinc-900 rounded-xl shadow-lg">
        <h1 className="text-3xl font-bold mb-2 text-black dark:text-white">K8s DaemonSets</h1>
        <form
        ...
        {fetchKey >= 0 && (
          <Suspense fallback={<div className="text-gray-500">Loading daemonsets...</div>}>
            <DaemonSetsFetcher key={namespace + fetchKey} namespace={namespace} />
          </Suspense>
        )}
      </main>
    </div>
